...
impl EntityReference for crate::Tenant {
...
#[derive(Debug)]
pub struct TenantRequest<R = crate::Tenant> {
    query: SelectQuery,
...
impl<R> Clone for TenantRequest<R> {
    fn clone(&self) -> Self {
        Self {
...
impl<R> TenantRequest<R> {
    ...
    pub fn return_type<T>(self) -> TenantRequest<T> {
        TenantRequest {
            query: self.query,
    ...
    pub fn which_are_enabled(mut self) -> Self {
        self.query = self.query.and_filter(Expr::eq("enabled", true));
        self
    }
...
impl<R: teaql_core::Entity> crate::PurposedQuery<TenantRequest<R>> {
    pub fn comment(mut self, comment: impl Into<String>) -> Self {
