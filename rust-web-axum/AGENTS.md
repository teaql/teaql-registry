...
```bash
cargo teaql --input models/nexus-repository-service.xml rust-assist-[action]/[entity-name]
```

> `models/nexus-repository-service.xml` is the default model path. If the model file is located elsewhere, adjust the `--input` path to match the actual file location in this project.
...
| blob_store_type | Blob Store Type |
| user_status | User Status |
| blob_store_configuration | Blob Store Configuration |
...
| security_privilege | Security Privilege |
