...
impl<'a> ContentRepositoryExpression<'a> {
    ...
    pub fn get_repository_id(self) -> crate::ValueExpression<'a, u64> {
        let next = self.result.and_then("repository_id", |entity| entity.eval_repository_id());
        crate::ValueExpression::new(next, self.root_desc.clone())
    }
    pub fn get_repository(self) -> crate::RepositoryConfigurationExpression<'a> {
        let next = self.result.and_then("repository", |entity| entity.eval_repository());
