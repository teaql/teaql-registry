...
impl<R> RepositoryConfigurationRequest<R> {
    ...
    fn apply_dynamic_json_chain_filter(self, head: &str, tail: &str, value: &JsonValue) -> Self {
        ...
        match head {
            ...
            "content_repository_list" => {
                self.with_content_repository_list_matching(
    ...
    pub fn have_content_repositories(self) -> Self {
        self.with_content_repository_list_matching(SelectQuery::new("ContentRepository"))
    }

    pub fn have_no_content_repositories(self) -> Self {
        self.without_content_repository_list_matching(SelectQuery::new("ContentRepository"))
    }

    pub fn with_content_repository_list_matching(mut self, request: impl Into<QuerySelection>) -> Self {
        let selection = request.into();
        self.query = self.query.and_filter(Expr::in_subquery(
            "id",
        ...
        self.relation_filters.push(RelationFilter::new("content_repository_list", selection));
        self
    }

    pub fn without_content_repository_list_matching(mut self, request: impl Into<QuerySelection>) -> Self {
        let selection = request.into();
        self.query = self.query.and_filter(Expr::not_in_subquery(
            "id",
            <crate::ContentRepository as teaql_core::TeaqlEntity>::entity_descriptor(),
    ...
    pub fn select_content_repository_list_with(mut self, request: impl Into<QuerySelection>) -> Self {
        let selection = request.into();
        self.query = self.query.relation_query("content_repository_list", selection.clone().into_query());
        self.relation_selections.push(RelationSelection::new("content_repository_list", selection));
        self
}
    ...
    pub fn count_content_repositories_with(mut self, alias: impl Into<String>, request: impl Into<QuerySelection>) -> Self {
        let selection = request.into();
    ...
    pub fn stats_from_content_repositories_as(mut self, alias: impl Into<String>, request: impl Into<QuerySelection>) -> Self {
        let selection = request.into();
        self.query_options.relation_aggregates.push(RelationAggregate::new(
            "content_repository_list",
            alias,
    ...
    pub fn group_by_content_repositories_with_details(self, request: impl Into<QuerySelection>) -> Self {
