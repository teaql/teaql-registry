    ...
    C: TeaqlRuntime + ?Sized + crate::TeaqlRepositoryProvider,
{
    ...
    ctx.user_context().transaction_data(|| async {
        Box::pin(generate_content_repositories(ctx, &mut state)).await.map_err(|e| {
            teaql_runtime::DataServiceError::Runtime(teaql_runtime::RuntimeError::Graph(e))
        })
    }).await.map_err(|e| e.to_string())?;
    ...
    C: TeaqlRuntime + ?Sized + crate::TeaqlRepositoryProvider,
{
        if state.ids("Nexus Platform").is_empty() {
            state.record_skipped(crate::SecurityPrivilege::ENTITY_NAME, "Required dependency Nexus Platform is missing in reference pool".to_string());
            log::info!("Skipped generating Security Privilege: Required dependency Nexus Platform is missing in reference pool.");
            return Ok(());
        }
...
{
        if state.ids("Nexus Platform").is_empty() {
            state.record_skipped(crate::SecurityRole::ENTITY_NAME, "Required dependency Nexus Platform is missing in reference pool".to_string());
            log::info!("Skipped generating Security Role: Required dependency Nexus Platform is missing in reference pool.");
            return Ok(());
        }
...
{
        if state.ids("Nexus Platform").is_empty() {
            state.record_skipped(crate::BlobStoreConfiguration::ENTITY_NAME, "Required dependency Nexus Platform is missing in reference pool".to_string());
            log::info!("Skipped generating Blob Store Configuration: Required dependency Nexus Platform is missing in reference pool.");
            return Ok(());
        }
...
{
        if state.ids("Nexus Platform").is_empty() {
            state.record_skipped(crate::SecurityUser::ENTITY_NAME, "Required dependency Nexus Platform is missing in reference pool".to_string());
            log::info!("Skipped generating Security User: Required dependency Nexus Platform is missing in reference pool.");
            return Ok(());
        }
...
{
        if state.ids("Nexus Platform").is_empty() {
            state.record_skipped(crate::RepositoryConfiguration::ENTITY_NAME, "Required dependency Nexus Platform is missing in reference pool".to_string());
            log::info!("Skipped generating Repository Configuration: Required dependency Nexus Platform is missing in reference pool.");
            return Ok(());
        }
    ...
    for i in 0..fanout {
        ...
        state.add_reference(crate::RepositoryConfiguration::ENTITY_NAME, entity.id().into_u64());
    }
...
where
    C: TeaqlRuntime + ?Sized + crate::TeaqlRepositoryProvider,
{
        if state.ids("Repository Configuration").is_empty() {
            state.record_skipped(crate::ContentRepository::ENTITY_NAME, "Required dependency Repository Configuration is missing in reference pool".to_string());
            log::info!("Skipped generating Content Repository: Required dependency Repository Configuration is missing in reference pool.");
            return Ok(());
        }
    ...
    log::info!("Generating sample data for Content Repository (expected: {})...", fanout);

    for i in 0..fanout {
        let mut entity = Q::content_repositories().purpose("Init Sample Data").new_entity(ctx);
        let mut used_refs = std::collections::HashSet::new();

                if let Some(ref_id) = state.pick_unused_id("Repository Configuration", i as usize, &used_refs) {
                    entity.update_repository_id(ref_id);
                    used_refs.insert(ref_id);
                } else {
                    // Optional relation was missing in reference pool
                }
        ...
        state.add_reference(crate::ContentRepository::ENTITY_NAME, entity.id().into_u64());
    }
    ...
    C: TeaqlRuntime + ?Sized + crate::TeaqlRepositoryProvider,
{
    ...
    for i in 0..fanout {
        ...
        let mut used_refs = std::collections::HashSet::new();
                ...
                entity.update_version(format!("{} {}", "3.12.0", i + 1));
        ...
        state.add_reference(crate::Component::ENTITY_NAME, entity.id().into_u64());
    }
    ...
    C: TeaqlRuntime + ?Sized + crate::TeaqlRepositoryProvider,
{
        ...
        if state.ids("Component").is_empty() {
            state.record_skipped(crate::Asset::ENTITY_NAME, "Required dependency Component is missing in reference pool".to_string());
            log::info!("Skipped generating Asset: Required dependency Component is missing in reference pool.");
            return Ok(());
        }
    ...
    for i in 0..fanout {
        ...
        let mut used_refs = std::collections::HashSet::new();
                ...
                if let Some(ref_id) = state.pick_unused_id("Component", i as usize, &used_refs) {
                    entity.update_component_id(ref_id);
                    used_refs.insert(ref_id);
                } else {
                    // Optional relation was missing in reference pool
                }
