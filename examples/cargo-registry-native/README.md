...
This disposable fixture distinguishes a real Cargo dependency graph from a registry that merely accepts and downloads `.crate` archives. The child crate depends on the base crate through the local `teaql` registry; a fresh consumer must download both and print `NATIVE_CARGO_TRANSITIVE_PASS value=42`.
